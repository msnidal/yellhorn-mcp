...
async def test_list_resources(mock_request_context):
    ...
    from yellhorn_mcp.git_utils import list_resources
    
    with (
        patch("yellhorn_mcp.git_utils.run_github_command", new=AsyncMock()) as mock_gh,
        ...
        resources = await list_resources(mock_request_context, None, mock_gh)
        ...
        # Check workplan resources
        assert resources[0].name == "Workplan #123: Test Workplan 1"
        assert resources[1].name == "Workplan #456: Test Workplan 2"
        # Check judgement resources
        assert resources[2].name == "Judgement #789: Judgement: main..HEAD for Workplan #123"
        assert resources[3].name == "Judgement #987: Judgement: v1.0..feature for Workplan #456"
        ...
        resources = await list_resources(mock_request_context, "yellhorn_workplan", mock_gh)
        ...
        resources = await list_resources(mock_request_context, "yellhorn_judgement_subissue", mock_gh)
        assert len(resources) == 2  # Should return the 2 mocked judgement resources
        mock_gh.assert_called_once_with(
            mock_request_context.request_context.lifespan_context["repo_path"],
            [
                "issue",
        ...
        resources = await list_resources(mock_request_context, "different_type", mock_gh)
...
async def test_get_codebase_snapshot():
    ...
    mock_git = AsyncMock(return_value="file1.py\nfile2.py\nfile3.py")
...
async def test_get_default_branch():
    ...
    mock_git = AsyncMock(return_value="* remote origin\n  Fetch URL: https://github.com/user/repo.git\n  Push  URL: https://github.com/user/repo.git\n  HEAD branch: main")
    with patch("yellhorn_mcp.git_utils.run_git_command", mock_git):
        branch = await get_default_branch(Path("/mock/repo"))
        assert branch == "main"

    # Test fallback to main
    mock_git = AsyncMock(side_effect=[YellhornMCPError("Command failed"), "main exists"])
    with patch("yellhorn_mcp.git_utils.run_git_command", mock_git):
        branch = await get_default_branch(Path("/mock/repo"))
        assert branch == "main"

    # Test fallback to master
    mock_git = AsyncMock(side_effect=[YellhornMCPError("Command failed"), YellhornMCPError("Command failed"), "master exists"])
    with patch("yellhorn_mcp.git_utils.run_git_command", mock_git):
        branch = await get_default_branch(Path("/mock/repo"))
        assert branch == "master"

    # Test when all methods fail - should return "main" as fallback
    mock_git = AsyncMock(side_effect=YellhornMCPError("Command failed"))
    with patch("yellhorn_mcp.git_utils.run_git_command", mock_git):
        branch = await get_default_branch(Path("/mock/repo"))
        assert branch == "main"
...
def test_format_metrics_section():
    ...
    result = format_metrics_section(model, None)
    ...
    result = format_metrics_section(model, partial_metadata)
    assert "**Model Used**: N/A" in result  # No total tokens, so it's invalid
...
async def test_create_workplan(mock_request_context, mock_genai_client):
    ...
    # Mock GitHub commands to return appropriate responses
    async def mock_github_command(repo_path, args):
    ...
    with (
        patch("yellhorn_mcp.server.run_github_command", new=AsyncMock(side_effect=mock_github_command)) as mock_gh,
        patch("yellhorn_mcp.git_utils.run_github_command", new=AsyncMock(side_effect=mock_github_command)) as mock_gh_utils,
        patch("yellhorn_mcp.git_utils.add_github_issue_comment", new=AsyncMock()) as mock_add_comment,
        patch("asyncio.create_task") as mock_create_task,
    ):
        # Set the github_command_func in the context to use our mock
        mock_request_context.request_context.lifespan_context["github_command_func"] = mock_github_command
        ...
        # Verify that the async task was created for workplan processing
        ...
        # Verify that no async task was created for AI processing (codebase_reasoning="none")
...
async def test_get_github_issue_body():
    ...
    result = await get_github_issue_body(Path("/mock/repo"), issue_url, mock_gh)
    ...
    result = await get_github_issue_body(Path("/mock/repo"), issue_number, mock_gh)
...
async def test_get_github_pr_diff():
    ...
    result = await get_github_pr_diff(Path("/mock/repo"), pr_url, mock_gh)
...
async def test_post_github_pr_review():
    ...
    with (
        ...
        result = await post_github_pr_review(Path("/mock/repo"), pr_url, "Review content", mock_gh)
...
async def test_add_github_issue_comment():
    ...
    with patch("yellhorn_mcp.git_utils.run_github_command") as mock_gh:
        ...
        await add_github_issue_comment(Path("/mock/repo"), "123", "Comment content", mock_gh)
...
async def test_update_github_issue():
    ...
    with (
        ...
        await update_github_issue(Path("/mock/repo"), "123", "Updated content", mock_gh)
...
async def test_process_workplan_async(mock_request_context, mock_genai_client):
    ...
    with (
        ...
        # Check that LLMManager was called
        mock_llm_manager.call_llm_with_citations.assert_called_once()

        # Get args from the call
        args, kwargs = mock_llm_manager.call_llm_with_citations.call_args
        ...
        expected_usage_metadata = mock_llm_manager.call_llm_with_citations.return_value["usage_metadata"]
        mock_format_metrics.assert_called_once_with("gemini-model", expected_usage_metadata)
        ...
        assert "Generated workplan content" in update_content
...
async def test_process_workplan_async_empty_response(mock_request_context, mock_genai_client):
    ...
    with (
        ...
        # Check that update_github_issue was called with empty content
        mock_update.assert_called_once()
        args, kwargs = mock_update.call_args
        assert args[0] == Path("/mock/repo")
        assert args[1] == "123"
        # The content should have title but empty workplan content
        update_content = args[2]
        assert "# Feature Implementation Plan" in update_content
        # Should have metrics section with N/A values due to no usage metadata
        assert "## Completion Metrics" in update_content
        assert "**Model Used**: N/A" in update_content

        # Verify add_github_issue_comment was not called
        mock_add_comment.assert_called_once()
...
async def test_get_workplan(mock_request_context):
    ...
    with patch("yellhorn_mcp.server.get_github_issue_body") as mock_get_issue:
        ...
        mock_get_issue.assert_called_once_with(
            mock_request_context.request_context.lifespan_context["repo_path"], 
            "123",
            mock_request_context.request_context.lifespan_context["github_command_func"]
        )
                ...
                with patch("yellhorn_mcp.server.create_github_subissue") as mock_create_subissue:
                    with patch("yellhorn_mcp.server.add_github_issue_comment") as mock_add_comment:
                        with patch("asyncio.create_task") as mock_create_task:
                            ...
                            # get_github_issue_body now takes github_command_func as third parameter
                            mock_get_issue.assert_called_once_with(repo_path, "123", mock_request_context.request_context.lifespan_context["github_command_func"])
                ...
                with patch("yellhorn_mcp.server.create_github_subissue") as mock_create_subissue:
                    ...
                    with patch("yellhorn_mcp.server.add_github_issue_comment") as mock_add_comment:
                        with patch("asyncio.create_task") as mock_create_task:
                            ...
                            # Parse the JSON result
                            result_data = json.loads(result)
                            assert result_data["subissue_url"] == "https://github.com/user/repo/issues/999"
...
async def test_process_workplan_async_search_grounding_enabled(
    ...
    with (
        ...
        patch("yellhorn_mcp.server.add_citations_from_metadata") as mock_add_citations,
    ):
        mock_snapshot.return_value = (["file1.py"], {"file1.py": "content"})
        mock_format.return_value = "Formatted codebase"
        mock_format_metrics.return_value = (
        ...
        mock_llm_manager.call_llm_with_citations = AsyncMock(
            return_value={
                "content": "Generated workplan content",
                "grounding_metadata": MagicMock(),  # Provide grounding metadata for citations
                "usage_metadata": {
                    "prompt_token_count": 1000,
                    "candidates_token_count": 500,
                    "total_token_count": 1500
        ...
        # Verify that LLMManager's call_llm_with_citations was called correctly
        mock_llm_manager.call_llm_with_citations.assert_called_once()
        call_args = mock_llm_manager.call_llm_with_citations.call_args
        # Check that generation_config was passed with tools
        assert "generation_config" in call_args[1]
        generation_config = call_args[1]["generation_config"]
        assert generation_config.tools == mock_search_tools
        # Check that temperature was set
        assert call_args[1]["temperature"] == 0.0
        # Check model
        assert call_args[1]["model"] == "gemini-2.5-pro-preview-05-06"

        # Verify citations processing was called (since we provided grounding_metadata)
        mock_add_citations.assert_called_once()
        
        # Verify the grounding metadata was passed to citations
        grounding_metadata = mock_llm_manager.call_llm_with_citations.return_value["grounding_metadata"]
        mock_add_citations.assert_called_with("Generated workplan content", grounding_metadata)
...
async def test_process_judgement_async_search_grounding_enabled(
    ...
    with (
        ...
        # Mock codebase snapshot and formatting
        mock_snapshot.return_value = (["file1.py"], {"file1.py": "content"})
        mock_format.return_value = "Formatted codebase"
        ...
        # Mock citations processing (should not be called for call_llm_with_usage)
        ...
        # Create mock LLM manager
        mock_llm_manager = MagicMock()
        # Import UsageMetadata for proper mock
        ...
        mock_add_citations.assert_called_once()
        grounding_metadata = mock_llm_manager.call_llm_with_citations.return_value["grounding_metadata"]
        mock_add_citations.assert_called_with("## Judgement Summary\nImplementation looks good.", grounding_metadata)

        # Verify update_github_issue was called with the expected content
        mock_update_issue.assert_called_once()
        call_args = mock_update_issue.call_args
        body = call_args[0][2]
        
        # Check that the body contains the judgement content (with citations since it uses call_llm_with_citations)
        assert "## Judgement Summary" in body
        assert "Implementation looks good." in body
        # Should contain citations since call_llm_with_citations handles them
...
async def test_process_judgement_async_update_subissue(mock_request_context, mock_genai_client):
    ...
    with (
        ...
        patch("yellhorn_mcp.server.add_github_issue_comment") as mock_add_comment,
            ...
            # Verify completion comment was added (always posted)
            mock_add_comment.assert_called_once()
            completion_comment = mock_add_comment.call_args[0][2]
            ...
            # Verify both completion comment and debug comment were added
            assert mock_add_comment.call_count == 2
            
            # Check the debug comment (second call)
            debug_comment_args = mock_add_comment.call_args_list[1]
            assert debug_comment_args[0][1] == "789"  # subissue_to_update
            assert "Debug Information" in debug_comment_args[0][2]

            # Update assertions to match the content returned by the mock LLM manager
            assert "**Base Ref**: `main` (Commit: `abc1234`)" in body
            assert "**Head Ref**: `HEAD` (Commit: `def5678`)" in body
            assert "**Codebase Reasoning Mode**: `full`" in body
            assert "**AI Model**: `gemini-2.5-pro-preview-05-06`" in body
